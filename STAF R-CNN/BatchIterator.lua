...
local function retrieve_entry(set, idx)
  local fn = set.list[set.order[idx]]
  return fn
end
...
function BatchIterator:processImage(img, rois, debug)
  local cfg = self.cfg
...
function BatchIterator:load_and_process_image(rois, fn, color_space, examples_base_path, debug)
  local new_rois = deep_copy(rois) -- will be modified twice: 1) yuv 2) rgb (for debug), so copy it first
  -- load image, wrap with pcall since image net contains invalid non-jpeg files
  local status, img = pcall(function () return load_image(fn, color_space, examples_base_path) end)
  if not status then
  ...
  local img, new_rois = self:processImage(img, new_rois, debug)
...
function BatchIterator:nextTraining(count)
  ...
  local function try_add_next()
    ...
    num_img_scanned = num_img_scanned + 1
    local rois = deep_copy(self.ground_truth[fn].rois)   -- copy RoIs ground-truth data (will be manipulated)
  
    -- load image, wrap with pcall since image net contains invalid non-jpeg files
    local img, rois, img_size = self:load_and_process_image(rois, fn, cfg.color_space, cfg.examples_base_path, false)
    if img == nil then
      return 0
    end
    
    local ori_img, _ = self:load_and_process_image(rois, fn, 'rgb', cfg.examples_base_path, true)
    if img == nil then
      return 0
    end
    ...
    local negative = self.anchors:sampleNegative(img_rect, rois, cfg.negative_threshold, 3) 
    ...
    if false then
      ...
      for i=1,#negative do
        draw_rectangle(ori_img, negative[i][1], red)
      end
      local green = torch.Tensor({0,1,0})
      for i=1,#positive do
        draw_rectangle(ori_img, positive[i][1], green)
      end
      
      for i=1,#rois do
        draw_rectangle(ori_img, rois[i].rect, white)
      end
--      image.saveJPG(string.format((opt.save..'anchors/anchors%d.jpg'), self.training.i-1), ori_img)
      image.saveJPG(string.format('debug/mix/anchors/anchors%d.jpg', self.training.i-1), ori_img)
    end
  ...
  if #self.background.list > 0 then
    ...
    if status then
      img = self:processImage(img, nil, true)
...
--    for i,m in ipairs(rois) do
--      draw_rectangle(ori_img, m.rect, white)
--    end
--    image_lib.display({image=ori_img, legend='Processed Validation Image'})
    
          
    table.insert(batch, { img = img, rois = rois, ori_img = ori_img })
...
function BatchIterator:batch_validation(count)
  local cfg = self.cfg
  local batch = {}
  count = count or 100
  
  for i=1, count do
    local fn = retrieve_entry(self.validation, i)
    local rois = deep_copy(self.ground_truth[fn].rois)   -- copy RoIs ground-truth data (will be manipulated, e.g. scaled)
    
    local img, rois = self:load_and_process_image(rois, fn, cfg.color_space, cfg.examples_base_path, false)
    if img == nil then
      goto continue
    end
